...
def get_clean_description(text):
    ...
    # 1. Replace line breaks and paragraphs with actual newlines
    text = re.sub(r'<(br|p|/p|br /)>', '\n', text, flags=re.IGNORECASE)
    # 2. Strip ALL remaining HTML tags
    text = re.sub(r'<[^>]+>', '', text)
    # 3. Decode HTML entities like &quot; or &amp;
    text = html.unescape(text)
    # 4. Remove AniList meta-notes
...
async def post_cmd(u, c):
    try:
        ...
        # 1. Fetch Info (Checking Cache first)
        info = db.cache.find_one({"n": name_input.lower()})
        if not info:
            info = get_info(name_input)
            if info: db.cache.update_one({"n": name_input.lower()}, {"$set": info}, upsert=True)
        
        if not info: return await u.message.reply_text("❌ Anime not found.")

        # 2. Aggressive Clean for Synopsis
        desc = get_clean_description(info.get('description', ''))
        mid = len(desc)//2
        # Building the hidden spoiler style
        synopsis = f"{desc[:mid]}<tg-spoiler>{desc[mid:]}</tg-spoiler>"

        # 3. Image Memory Logic
        if user_img:
        ...
        # 4. Title & Quality Formatting
        alt = info['title'].get('romaji') or info['title'].get('native')
        title = f"{name_input} ({alt})" if alt and name_input.lower() != alt.lower() else name_input
        
        quals = re.findall(r'(\d+p|4K|360p)', links_text, re.I)
        ...
        await u.message.reply_text(f"<b>Error during parsing:</b>\n<code>{e}</code>", parse_mode='HTML')

async def callback(u, c):
    q = u.callback_query
    t = c.user_data.get('temp')
    if not t: return

    targets = [ch['cid'] for ch in db.channels.find({"uid": u.effective_user.id})] if q.data == "send_all" else [q.data.replace("sel_","")]
    for cid in targets:
        try: await c.bot.send_photo(chat_id=cid, photo=t['img'], caption=t['cap'], reply_markup=InlineKeyboardMarkup(t['kb']), parse_mode='HTML')
        except: pass
